...
def spectral(waveform, fs, NFFT=2 ** 9, overlap=0.5, winN="hanning", outF=None, logSpec=True):
    """
    ...
    specM, s2f, s2t = mlab.specgram(
        waveform, NFFT=NFFT, Fs=fs, window=win, noverlap=int(NFFT * overlap)
    )
...
def spectral_nDelta(waveform, fs, NFFT=2 ** 9, overlap=0.5, winN="hanning", order=1, logSc=True):
    # settings
    win = sig.get_window(winN, NFFT)  # plt.mlab.window_hanning\
    overlap = float(overlap)
    order = int(order)
...
def cepstral(waveform, fs, NFFT=2 ** 9, overlap=0.5, Nceps=2 ** 4, logSc=True, **kwargs):
    ...
    log_spec_y = librosa.core.logamplitude(
        librosa.core.spectrum._spectrogram(
            waveform, n_fft=NFFT, hop_length=int(NFFT * (1 - overlap), **kwargs)
        )[0]
    )
...
def mfcepstral(
    waveform, fs, NFFT=2 ** 9, overlap=0.5, Nceps=2 ** 4, logSc=True, n_mels=128, **kwargs
):
    """
...
def mfcepstral_nDdelta(
    waveform, fs, NFFT=2 ** 9, overlap=0.5, Nceps=2 ** 4, order=1, logSc=True, n_mels=128, **kwargs
):
    ...
    ## CEPSTROGRAM
    M0 = librosa.feature.mfcc(
        waveform, sr=fs, n_mels=n_mels, n_mfcc=Nceps, n_fft=NFFT, hop_length=hopSz, **kwargs
    )  # , hop_length=hopSz)
    m = np.shape(M0)[1]
...
def melSpectral_nDelta(
    waveform, fs, NFFT=2 ** 10, overlap=0.5, n_mels=2 ** 4, order=1, logSc=True, **kwargs
):
    ...
    ## CEPSTROGRAM
    Mc = librosa.feature.melspectrogram(
        waveform, sr=fs, n_mels=n_mels, n_fft=NFFT, hop_length=hopSz, **kwargs
    )
    Mc_log = np.log(Mc)
    M = Mc.copy()
    if logSc:
