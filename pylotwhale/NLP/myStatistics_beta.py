...
def plDist_with_obsValue(
    i, j, shuffledDistsM, obsM, ax=None, plTitle=None, kwargs_obs=None, **kwargs
):
...
def randomisation_test_repetitions(
    df_dict,
    Dtint,
    obsStat,
    Nsh,
    ...
    for i in range(Nsh):  # shuffle ith-loop
        ...
        for t in df_dict.keys():  # for each tape
            thisdf = df_dict[t]
            cfd_sh += shuffled_cfd(
                thisdf, Dtint, label=label, time_param=time_param
            )  # counts in current tape
...
def randomisation_test4bigrmas_inSequences(
    seqOfSeqs, df_obsStat, Nsh, condsLi, sampsLi, testStat=teStat_proportions_diff
):
    ...
    seq_slicer = superSequenceSlicer(
        seqOfSeqs
    )  # np.cumsum(np.array([len(item) for item in seqOfSeqs]))
    ...
    for i in np.arange(Nsh):
        ...
        sequences_str = aa.seqsLi2iniEndSeq(sliceBackSuperSequence(superSequence, seq_slicer))
        ...
        Mp_sh = ngr.condFreqDict2condProbMatrix(cfd_sh, condsLi, sampsLi)[0]  # normalised matrix
        shTest_i = testStat(Mp_sh)  # compute satat variable
        shuffle_tests[i] = shTest_i  # save distribution for later
...
def randomiseSeqOfSeqs(seqOfSeqs):
    ...
    seqOfSeqs_sh = sliceBackSuperSequence(superSequence, slicer)
...
class shuffleSequence(baseSequence):
    ...
    def randomise(self, seqOfSeqs):
        ...
        ## back to seq of seqs
        seqOfSeqs_sh = self.sliceBackSuperSequence(superSequence_sh, slicer)
    ...
    for r in range(nr)[:]:
        for c in range(nc)[:]:
            if X[r, c] >= min_counts:
                px = 1.0 * X[r, c] / nx[r]
                py = 1.0 * Y[r, c] / ny[r]
                XYtest[r, c] = testDiffProportions(px, py, nx[r], ny[r], pcValue=pcValue)[0]
            else:
                XYtest[r, c] = np.NaN
...
def CVbw_KDE(x, kde_param_grid=None, **CV_kwargs):
    ...
    grid = GridSearchCV(estimator=KernelDensity(), param_grid=kde_param_grid, **CV_kwargs)
    data = x[:, np.newaxis]
...
def fit_KDE_CVbw(x, supp_range, num=1000, kde_param_grid=None, **CV_kwargs):  #  bw_range=None):
