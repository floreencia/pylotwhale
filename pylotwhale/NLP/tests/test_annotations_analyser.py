...
d = {"call": ["A", "B", "C", "D", "A", "C", "E"], "t": [1.0, 1.5, 2.0, 2.0, 0.5, 10.0, np.nan]}
te_df = pd.DataFrame(d)

df_100 = pd.DataFrame(
    np.hstack((np.random.randint(0, 5, size=(100, 1)), np.random.rand(100, 1))), columns=list("AB")
)
...
def test_chunkStruct():
    ...
    Ngrams_dist = aa.Ngrams_distributionDt_ndarray(
        {"a": df_100}, Dtvec, seqLabel="A", time_param="B"
    )
    # calls in ngrams
