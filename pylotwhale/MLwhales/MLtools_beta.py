...
def colCounts2colFreqs(M):
    ...
    return np.divide(1.0 * M, 1.0 * M.sum(axis=0))
...
def plXy(X, y, figsize=None, cmapName_L="gray_r", cmapName_Fig="gray_r"):
    ...
    for label, ix_label in y_map.items():
        # line
        artistLi.append(plt.Line2D((0, 1), (0, 0), color=labels_cmap[ix_label], linewidth=14))
    ...
    axX.imshow(X, aspect="auto", interpolation="nearest", cmap=plt.cm.get_cmap(cmapName_Fig))
    ## labels
    ...
    axY.imshow(
        y_num, aspect="auto", cmap=cmap, interpolation="nearest"  # plt.cm.get_cmap(cmapName_L),
    )
...
def balanceToClass(X, y, class_label, random_state=1, shuffle_samples=False):
    ...
    for l in balance_labels:  # for each of the other classes
        ...
        sX, sy = resample(
            thisX, thisy, random_state=random_state, n_samples=class_n_samples, replace=False
        )
        ## stack
...
class dataX:
    ...
    def addInstances(self, newX):
        ...
        if self.X is None:
            ...
            assert (
                self.n_attr is None or n == self.n_attr
            ), "different feature spaces {} =/= {}".format(n, self.n_attr)
...
class dataXy_names(dataX):
    ...
    def addInstances(self, new_X, new_y_names):
        ...
        if self.X is None:  # first time we load data
            ...
            assert m == len(new_y_names), "inconsistent labelling {} =/= {}".format(
                m, len(new_y_names)
            )
