...
"""
Created on Tue Nov  3 17:08:19 201
@author: florencia
...
def parseHeikesNameConv(fileN):
    ...
    try: # second try, try to parse (2) - whale group and call type        
        ...
        return di
    except: #ValueError or AttributeError: # not even possible to parse call type
           #print("ERROR: filename doesn't match Heike's convention!\n\t%s"%bN)
           #print("    eg. NPW-033-J-B-090713_f50-8_00_01_52...")
           return None   
...
def timeStamp2seconds(timeStamp):
    '''
    parses timestamps with the format hh_mm_ss and returns the timestampstamp in seconds
    '''
    try:
...
def c_Label2HeikesCallLabel(annFile, labKey='call', repL = 'c'):
    '''annotation-files with a single section, preplace c with the call label in the file name'''
