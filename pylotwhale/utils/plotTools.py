...
def stackedBarPlot(
    ...
    for i in range(len(freq_arr)):
        arr = freq_arr[i]
        ax.bar(ind, arr, bottom=arr0, color=next(color), label=key_labels[i])
        arr0 += arr
...
def display_numbers(fig, ax, M, fontSz, fmt=int, condition=lambda x: True):
    ...
    for i in range(r):
        for j in range(c):
            if condition(M[i, j]):
                ax.text(x=j, y=i, s=fmt(M[i, j]), va="center", ha="center", fontsize=fontSz)
    return fig, ax
...
def plImshowLabels(
    ...
    **kwarg
    ...
    if yLabel:
        ax.set_ylabel(yLabel)
    # COLOR BAR
    if isinstance(cbarLim, tuple):
        im.set_clim(cbarLim)  # cmap lims
    ...
    if cbar:
        ...
        if isinstance(cbarLim, tuple):
            cbar.set_clim(cbarLim)  # cbar limits
        if isinstance(cbarTicks, list):
            cbar.set_ticks(cbarTicks)
        if isinstance(cbarTickLabels, list):
...
def plspectro(
    ...
    **kwargs
    ...
    # zeros the the spectral energy smaller than 0.001% of <E>
    A = reScale_E(A, spec_factor=spec_fac)
    ...
    cax = ax.imshow(
        np.log(A),
        extent=[0, tf, v_cut[0] / 1000, v_cut[1] / 1000],
        origin="lower",
        aspect="auto",
...
def plspectro_cbar(ax):
    ...
    cbar.ax.set_yticklabels(
        ["10$^{:d}$".format(ll), "10$^{:d}$".format(ml), "10$^{:d}$".format(ul)]
    )
