...
files_df["annSections"] = [
    len(open(os.path.join(annsDir, annFi), "r").readlines()) for annFi in files_df["file_name"]
]
...
def test_data():
    ...
    assert set(files_df.columns) == set(
        ["monkey", "file_name", "predator", "sex", "tape", "annSections"]
    )
...
call_df = pd.DataFrame(
    columns=["call", "t0", "tf", "ici0", "ioi", "cl", "file_name", "tag", "Dtag", "ici"]
)  #'tape' , 'bird', 'area',
...
df = pd.merge(
    call_df,
    files_df[["file_name", "tape", "monkey", "predator", "sex"]],
    how="left",
    on=["file_name"],
)
